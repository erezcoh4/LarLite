...
namespace larlite {
    ...
    bool AnaPandoraNuTracks::analyze(storage_manager* storage) {
        bool DoPrint = false;
        bool CreateImagas = true;
        bool calculate_adc_in_corners = false;
        TString images_path = "/Users/erezcohen/Desktop/uBoone/BDTprotonCandidates/images";
...
//             if (run != 5905) return false;
            ...
            for (auto const& t: *ev_track){
                ...
                if (ev_wire){
                    ...
                    if (CreateImagas) {
                        hTrackROI = new TH2F(Form("r%d_s%d_e%d_t%d",run,subrun,event,track_id)
                                             ,Form("r%d s%d e%d t%d window in collection plane (%d,%.1f)=>(%d,%.1f)"
                                                   ,run,subrun,event,track_id,b._start_wire,b._start_t,b._end_wire,b._end_t)
                                             , b._end_wire - b._start_wire + 1  , b._start_wire , b._end_wire
                                             , (int)(b._end_t - b._start_t) + 1  , b._start_t , b._end_t);
                        hTrackROI -> GetXaxis() -> SetTitle("wire number");
                        hTrackROI -> GetXaxis() -> CenterTitle();
                        hTrackROI -> GetYaxis() -> SetTitle("time tick");
                        hTrackROI -> GetYaxis() -> CenterTitle();
                        hTrackROIzoomout = new TH2F(Form("r%d_s%d_e%d_t%d_zoomout",run,subrun,event,track_id)
                                                    ,Form("r%d s%d e%d t%d window in collection plane (%d,%.1f)=>(%d,%.1f)"
                                                          ,run,subrun,event,track_id,b._start_wire-20,b._start_t-30,b._end_wire+20,b._end_t+30)
                                                    , b._end_wire - b._start_wire + 41  , b._start_wire-20 , b._end_wire+20
                                                    , (int)(b._end_t - b._start_t) + 61  , b._start_t-30 , b._end_t+30);
                        hTrackROIzoomout -> GetXaxis() -> SetTitle("wire number");
                        hTrackROIzoomout -> GetXaxis() -> CenterTitle();
                        hTrackROIzoomout -> GetYaxis() -> SetTitle("time tick");
                        hTrackROIzoomout -> GetYaxis() -> CenterTitle();
                    }
                    ...
                    for (auto const& wire : *ev_wire) {
                        ...
                        if ( plane == 2 ) {
                            ...
                            if (CreateImagas) {
                                ...
                                if ( b._start_wire - 20 <= detWire && detWire <= b._end_wire + 20 ) {
                                    for (auto & iROI : wire.SignalROI().get_ranges()) {
                                        const int FirstTick = iROI.begin_index();
                                        time_tick = FirstTick;
                                        for (float ADC : iROI) {
                                            if ( b._start_t - 30 <= time_tick && time_tick <= b._end_t + 30 ) {
                                                hTrackROIzoomout -> SetBinContent( detWire - (b._start_wire - 20) , time_tick - (b._start_t - 30) , ADC );
                                            }
                                            time_tick++;
                                        }
                                    }
                                ...
                                // accumulate the ADC signal around the left bottom corner of the track ROI box
                                if ( b._start_wire <= detWire && detWire <= b._end_wire ) {
                                    CreatingTrackImage = true;
                                    for (auto & iROI : wire.SignalROI().get_ranges()) {
                                        const int FirstTick = iROI.begin_index();
                                        time_tick = FirstTick;
                                        for (float ADC : iROI) {
                                            if ( b._start_t <= time_tick && time_tick <= b._end_t ) {
                                                hTrackROI -> SetBinContent( detWire - b._start_wire , time_tick - b._start_t , ADC );
                                            }
                                            time_tick++;
                                        }
                                    }
                ...
                if (CreatingTrackImage) {
                    cout << "create image: " << Form("r%d_s%d_e%d_t%d.pdf",run,subrun,event,track_id) << endl;
                    ...
                    TCanvas * cTmp1 = new TCanvas("track ROI zoomout");
                    hTrackROIzoomout -> Draw("colz");
                    cTmp1 -> SaveAs(Form("%s/r%d/s%d_e%d_t%d/t%d_zoomout.pdf",images_path.Data(),run,subrun,event,track_id,track_id));
                    TCanvas * cTmp2 = new TCanvas("track ROI");
                    hTrackROI -> Draw("colz");
                    cTmp2 -> SaveAs(Form("%s/r%d/s%d_e%d_t%d/t%d_ROI.pdf",images_path.Data(),run,subrun,event,track_id,track_id));
                    delete cTmp1;
                    delete cTmp2;
                }
